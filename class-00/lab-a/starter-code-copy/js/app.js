...
function Product(name) {
  this.name = name;
  this.path = `img/${name}.jpg`//'img/' + name + '.jpg';
  this.votes = 0;
  this.views = 0;
...
function displayPics(){
  ...
  // rando is being called outside of its scope.
  console.log(viewed);
...
function handleClick(event) {
  ...
  for(const i = 0; i < names.length; i++){
    if(event.target.id === allProducts[i].name) {
      ...
      console.log(`${event.target.id} has ${allProducts[i].votes} votes in ${allProducts[i].views} views`); //event.target.id + ' has ' + allProducts[i].votes + ' votes in ' + allProducts[i].views + ' views');
    }
  }
...
function showList() {
  for(const i = 0; i < allProducts.length; i++) {
    ...
    liEl.textContent = `${allProducts[i].name} has ${allProducts[i].votes} votes in ${allProducts[i].views} views`;//allProducts[i].name + ' has ' + allProducts[i].votes + ' votes in ' + allProducts[i].views + ' views';
